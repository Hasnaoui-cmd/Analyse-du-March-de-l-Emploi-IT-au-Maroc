...
def ingerer_bronze(filepath_source: str, data_lake_root: str) -> dict:
    ...
    """
    print("Démarrage de l'ingestion vers la zone BRONZE...")
    ...
    partitions = {}
    for offre in offres:
        source = offre.get('source', 'inconnu').lower().replace(' ', '')
        date_pub = offre.get('date_publication', '')

        try:
            # Extract YYYY_MM from "YYYY-MM-DD"
