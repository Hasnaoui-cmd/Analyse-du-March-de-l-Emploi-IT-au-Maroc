...
def charger_depuis_bronze(data_lake_root: str) -> pd.DataFrame:
    ...
    print("Démarrage de la transformation vers la zone SILVER...")
    all_offres = []
...
def nettoyer_titres_postes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Nettoyage de la ville standardisée
    df['ville_std'] = df['ville'].str.title().str.strip()
    
    # Nettoyage du type de contrat
    mapping_contrat = {
        'cdi': 'CDI', 'contrat à durée indéterminée': 'CDI', 'permanent': 'CDI',
        'freelance': 'Freelance', 'cdd': 'CDD'
    }
    df['type_contrat_std'] = df['type_contrat'].str.lower().str.strip().map(mapping_contrat).fillna('Non spécifié')
    
    # --- FIX: Extract Year and Month for DuckDB ---
    # Convert 'YYYY-MM-DD' to string and slice it
...
def normaliser_experience(df: pd.DataFrame) -> pd.DataFrame:
    """Transforme l'expérience en valeur numérique (années min/max requises)."""
    def parser_experience(valeur):
        ...
        fourchette = re.search(r'(\d+)\s*[-à]\s*(\d+)', s)
        if fourchette:
